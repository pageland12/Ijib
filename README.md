...
### 4. 주요 데이터 파일 및 사용법 (Data Setup)

* **`이집어때_DB.sql`**: Oracle DB 테이블 생성 및 프로젝트에 필요한 데이터베이스 구조를 구성하기 위한 SQL 스크립트
  * 회원, 음식점, 메뉴, 북마크, 구독권, 결제, 게시판, 리뷰 등의 테이블 생성
  * **사용법**: SQL Developer에서 해당 스크립트를 실행하여 프로젝트에 필요한 테이블을 생성합니다.

* **`100year_restaurants_final_keywords.csv`**: 크롤링 및 데이터 수집을 통해 확보한 백년가게 음식점 데이터셋
  * 음식점명, 음식 종류, 지역, 주소, 전화번호, 영업시간, 이미지, 메뉴, 음식점 설명, 주차 정보, 위도·경도, 키워드 등의 데이터 포함
  * 약 700여 개의 음식점 데이터를 포함

* **`음식점자동등록.py`**: Selenium을 활용하여 CSV에 저장된 음식점 데이터를 관리자 페이지에 자동으로 등록하는 Python 스크립트
  * 관리자 계정으로 로그인 후 음식점 등록 페이지에 접속
  * CSV 데이터를 읽어 음식점명, 이미지, 카테고리, 키워드, 설명, 주소, 위치 정보, 전화번호, 영업 정보, 주차 정보, 메뉴 등을 자동 입력
  * **사용법**:
    1. 스프링 부트 서버를 먼저 실행합니다.
    2. Python 환경에서 Selenium을 설치합니다.
       `pip install selenium`
    3. `100year_restaurants_final_keywords.csv`와 `음식점자동등록.py`를 동일한 경로에 위치시킵니다.
    4. `음식점자동등록.py`를 실행하면 관리자 페이지에 음식점 데이터가 자동으로 등록됩니다.
...
| **팀원 박보성** | 자료 수집 / 결제 / 챗봇 | • 백년가게 자료 수집 및 데이터 정제<br>• 결제·환불 API Controller, DAO, DTO, View 구현<br>• 챗봇 구현 |
