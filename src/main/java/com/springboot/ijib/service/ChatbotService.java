    ...
    private String callLLMApi(String query, List<StoreDTO> stores) {
        try {
        	...
        	String fullPrompt = 
					...
					"[추천 심사 우선순위 및 규칙]\n" +
					"1. 위치(주소) 최우선 적용: 사용자가 언급한 특정 지역(시/도, 시/군/구, 동/역 등)이 있다면 해당 주소지에 위치한 가게를 무조건 최우선으로 선정해. 키워드가 잘 맞더라도 사용자가 요청한 지역과 거리가 멀다면 제외해.\n" +
					"2. 키워드 및 상세설명 문맥 비교: 사용자가 제시한 핵심 키워드(예: 혼밥, 주차, 단체 등)가 식당 태그와 완전히 일치하지 않더라도, 식당의 [상세설명]과 [메뉴] 내용을 꼼꼼히 비교해 유사하거나 호환되는 문맥(예: '혼밥' ↔ '1인 테이블/단품 메뉴', '주차' ↔ '인근 주차 공간')이 있다면 적극 반영해줘.\n" +
					"3. 매칭 미흡 시 대응: 요청한 세부 조건에 100% 들어맞는 곳이 부족하더라도 타 지역으로 넘어가지 말고, 해당 지역 내에서 가장 대중적이고 평가가 우수한 백년가게를 대체 추천하면서 이유를 안내해줘.\n\n" +
